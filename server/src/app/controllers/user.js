...
const login = asyncHandler(async (req, res) => {
    ...
    if (response && (await response.isCorrectPassWord(password)) ) {
        ...
        if(isBlocked) throw new Error('Tài khoản của bạn đã bị khóa !');
...
const updateAddress = asyncHandler(async (req, res) => {
    ...
    const response = await User.findByIdAndUpdate(id, { $set: { address: req.body.address } }, { new: true });
    return res.status(200).json({
        success: response ? true : false
    });
});
...
const getCart = asyncHandler(async (req, res) => {
    const userId = req.user.id;
    const user = await User.findById(userId).select('cart').populate('cart.product', exclude);
    if (!user) {
        return res.status(404).json({
            success: false,
            message: 'User not found'
